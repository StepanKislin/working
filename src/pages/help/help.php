...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['question'])) {
    ...
    foreach ($qa_list as $row) {
        ...
        foreach ($patterns as $pattern) {
            if (mb_strpos($clean_input, mb_strtolower(trim($pattern), 'UTF-8')) !== false) {
                $matches++;
            }
        }
...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;font-family:'Segoe UI',Tahoma,Geneva,Verdana,sans-serif}
        ...
        .logo{font-size:1.8rem;font-weight:bold;color:#4a9eff;text-decoration:none}
        .search-box{flex:1;max-width:500px;margin:0 2rem}
        .search-box input{width:100%;padding:0.6rem 1rem;border-radius:2rem;border:1px solid #444;background:#2a2a2a;color:white}
        .profile-link{text-decoration:none;color:#4a6cf7;font-weight:600}
        .user-menu{position:relative;display:flex;align-items:center}
        .profile-btn{background:none;border:none;color:#4a6cf7;font-weight:600;font-size:1rem;cursor:pointer;padding:0.5rem 1rem;border-radius:6px;transition:background .2s}
        .profile-btn:hover{background:rgba(74,108,247,.1)}
        .dropdown-menu{display:none;position:absolute;top:100%;right:0;background:#222;border:1px solid #444;border-radius:8px;box-shadow:0 4px 12px rgba(0,0,0,.3);min-width:180px;z-index:1000;flex-direction:column}
        .dropdown-menu a{padding:.75rem 1rem;color:#e0e0e0;text-decoration:none;font-size:.95rem;transition:background .2s,color .2s}
        .dropdown-menu a:hover{background:#333;color:#4a9eff}
        .dropdown-menu.show{display:flex}
        .logout-link{color:#ff6b6b!important}
        .logout-link:hover{color:#ff4d4d!important}
        .content-area{padding:2rem;min-height:calc(100vh - 120px);max-width:800px;margin:0 auto}
        .help-header{text-align:center;margin-bottom:2rem}
        .help-header h1{font-size:2.2rem;color:#4a9eff;margin-bottom:.5rem}
        .help-header p{color:#aaa;font-size:1.1rem}
        .ai-chat{background:#1a1a1a;border-radius:16px;padding:1.5rem;box-shadow:0 4px 20px rgba(0,0,0,.4);position:relative}
        .ai-message{background:#2a2a2a;padding:1.2rem;border-radius:12px;margin-bottom:1.5rem;line-height:1.6;opacity:0;animation:fadeIn .4s forwards}
        .ai-message strong{color:#4a9eff;display:block;margin-bottom:.4rem}
        .thinking{background:#2a2a2a;padding:1.2rem;border-radius:12px;margin-bottom:1.5rem;line-height:1.6;color:#aaa}
        .thinking span{display:inline-block;animation:blink 1.2s infinite}
        .thinking span:nth-child(2){animation-delay:.2s}
        .thinking span:nth-child(3){animation-delay:.4s}
        @keyframes blink{0%,60%,100%{opacity:.2}30%{opacity:1}}
        @keyframes fadeIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:translateY(0)}}
        .question-input{display:flex;gap:.8rem}
        .question-input input{flex:1;padding:.8rem 1.2rem;border-radius:30px;border:1px solid #444;background:#2a2a2a;color:white;font-size:1rem}
        .question-input button{padding:.8rem 1.5rem;border-radius:30px;background:#4a6cf7;color:white;border:none;font-weight:600;cursor:pointer;transition:background .2s}
        .question-input button:hover{background:#5a7cff}
        .examples{margin-top:1.5rem;color:#aaa;font-size:.95rem}
        .examples p{margin-bottom:.4rem}
        .examples span{color:#4a9eff;cursor:pointer;text-decoration:underline}
        .examples span:hover{color:#6aa9ff}
    </style>
</head>
<body>
...
<div class="content-area">
    ...
    <div class="ai-chat" id="ai-chat">
        ...
        <div class="question-input">
            <input type="text" id="user-question" placeholder="Например: Как пройти карьерный тест?" onkeypress="if(event.key==='Enter') askAI()">
...
function askAI() {
    ...
    .catch(() => {
        ...
        const aiMsg = document.createElement('div');
        aiMsg.className = 'ai-message';
        aiMsg.innerHTML = '<strong>ИИ-ассистент:</strong> Произошла ошибка. Попробуйте позже.';
        aiChat.insertBefore(aiMsg, aiChat.querySelector('.question-input'));
    });
